    ...
    public void SendCommand(final int cmd,final String data,final int cmd_type)
    {
        new Thread() {
            public void run() {
                ...
                Log.d(TAG, "SendCommand-message:" + message + " ,msg_length:" + msg_length + ", local:" + local + ", server_port:" + server_port + ", port:" + p.getPort() + ", p.getData:" + p.getData());

                try {
                    s.send(p);
                    Log.d(TAG, "s.send(p):" + message);
                    s.close();
                } catch (IOException e) {
